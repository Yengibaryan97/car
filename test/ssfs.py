...
def sell(self, car_model):
    message = f"I have not {car_model}"
...
class Buyer(Person):
    ...
    def change_money(self, selller, car_model):
        print(f"{self.name} buy this car")
        self.money = self.money - Seller.car_price(self, car_model)
        self.spent_money = self.spent_money + Seller.car_price(self, car_model)
        return f"{self.name} spent {self.spent_money} and {self.name} have already had {self.money} dollar and {selller} get {self.spent_money} dollar for {car_model}"

    def buy(self, seller, wanted_car):
        seller.sell(wanted_car)
        if wanted_car in Seller.car_park and self.money > Seller.car_price(self, wanted_car):
            return self.change_money(seller.name, wanted_car)
        elif wanted_car in Seller.car_park and self.money < Seller.car_price(self, wanted_car):
